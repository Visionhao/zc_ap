...
<html>
    ...
    <script type="text/javascript">
            function onInvokeAction(id) { 
				...
				location.href = "${pageContext.request.contextPath}/view/admin/privilege/userFucLimit/Down2-list.action?QA_PARENT_ROW_KEY=<%=request.getAttribute("QA_PARENT_ROW_KEY") %>&QT_fusCatalogId=<%=request.getAttribute("QT_fusCatalogId") %>&QT_chnName=<%=request.getAttribute("QT_chnName") %>&QT_engName=<%=request.getAttribute("QT_engName") %>&" + parameterString;
            }
            function onInvokeExportAction(id) {
                var parameterString = $.jmesa.createParameterStringForLimit(id);
                location.href = '${pageContext.request.contextPath}/view/admin/privilege/userFucLimit/Do-list.action?' + parameterString;
            }
	</script>      
    </head>
    <body>
        <form name="tableId" action="#">
            ...
            <jmesa:htmlTable captionKey="presidents.caption" width='99%' >		
                <jmesa:htmlRow>	 
                    ...
                    <jmesa:htmlColumn property="effectDate" title="生效时间"  pattern="yyyy-MM-dd" cellEditor="org.jmesa.view.editor.DateCellEditor"  sortable="false" filterable="false" editable="false"/>  
                    <jmesa:htmlColumn property="expireDate" title="失效时间"  pattern="yyyy-MM-dd" cellEditor="org.jmesa.view.editor.DateCellEditor"  sortable="false" filterable="false" editable="false"/>  
               </jmesa:htmlRow>
            </jmesa:htmlTable> 
            </jmesa:tableModel>
        </form>
