...
<html>
    ...
    <script type="text/javascript">
            function onInvokeAction(id) { 
                ...
                location.href="${pageContext.request.contextPath}/view/paramCodeTreeListTemplate/Do-list.action?QT_num=<%=request.getAttribute("QT_num") %>&QT_type=<%=request.getAttribute("QT_type") %>&QT_mnemonicCode=<%=request.getAttribute("QT_mnemonicCode") %>&QT_codeNum=<%=request.getAttribute("QT_codeNum") %>&QT_typeCode=<%=request.getAttribute("QT_typeCode") %>&QT_userShowName=<%=request.getAttribute("QT_userShowName") %>&QT_status=<%=request.getAttribute("QT_status") %>&QT_parentCodeNum=<%=request.getAttribute("QT_parentCodeNum") %>&"+parameterString;
            }
            function onInvokeExportAction(id) {
                var parameterString = $.jmesa.createParameterStringForLimit(id);
                location.href = '${pageContext.request.contextPath}/view/paramCodeTreeListTemplate/Do-list.action?' + parameterString;
            }
        ...
        <form style="margin-right: 4px;" name="tableId" action="#">
            ...
            <jmesa:htmlTable captionKey="presidents.caption" width='100%' >		
                <jmesa:htmlRow>	 
            		...
            		<input type="checkbox" name="items"  value="${bean.rowId}" onclick="this.parentNode.parentNode.style.backgroundColor=this.checked?'#80bdff':'#ffffff'"/> 
            		</jmesa:htmlColumn>
            		<jmesa:htmlColumn property="rowId" title="代码标识" sortable="false" filterable="false" editable="false"/>
                    <jmesa:htmlColumn property="codeNum" title="编号" sortable="false" filterable="false" editable="false"/>
                    <jmesa:htmlColumn  property="typeCode" title="类别编号"   filterable="false" editable="false" sortable="false"/> 
                    ...
                    <jmesa:htmlColumn property="parentCodeNum" title="上级代码编号" sortable="false" filterable="false" editable="false"/> 
