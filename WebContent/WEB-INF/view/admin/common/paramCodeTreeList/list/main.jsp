...
<script type="text/javascript">
			...
			function del(ids){
				$.ajax({  
		            url:'${pageContext.request.contextPath}/view/paramCodeTreeListTemplate/Do-delete.action',//先走校验的action   
		            type:"POST",          
		            data:"id="+ids,       
		            dataType:'text',  
			...
			function checkTheForm(){
				document.paramCodeTreeListForm.submit();
			}
			...
			function getIfrmaeCheckValue(objThis,iframeName,checkName,opt){
				var skip = 0;
				var modifyUrl = "${pageContext.request.contextPath}/view/paramCodeTreeListTemplate/Do-editPage.action?id=";
				var ids = "";
				 ...
				 if(opt=='edit'){
					 openwindow(comp,'修改参数代码',550,550);
				 }
				 if(opt=='show'){
					 var url = "${pageContext.request.contextPath}/view/paramCodeTreeListTemplate/Do-show.action?id="+ ids;
	            	 openwindow(url,'查看参数代码',550,500); 
				 }
			}
			$(function() {	
				$("#add-info").click(
						function() {
							openwindow('${pageContext.request.contextPath}/view/paramCodeTreeListTemplate/Do-addPage.action?nodePid=<%=request.getAttribute("nodePid") %>&QT_typeCode=<%=request.getAttribute("QT_typeCode") %>','新增参数代码',550,550); 
						});
			});
			function refresh (){
				var url = "${pageContext.request.contextPath}/view/paramCodeTreeListTemplate/Do-list_main.action";			
				window.location.href=url;
			}
</script>
</head>
<body >
<form name="paramCodeTreeListForm" id="paramCodeTreeListForm" action="<%=request.getContextPath()%>/view/paramCodeTreeListTemplate/Do-list.action?QT_codeNum=<%=request.getAttribute("QT_codeNum") %>&QT_typeCode=<%=request.getAttribute("QT_typeCode") %>&QT_parentCodeNum=<%=request.getAttribute("QT_parentCodeNum") %>&nodeId=<%=request.getAttribute("nodeId") %>" method="post" target="mainFrame">
		...
		<table id="searchArea" width="100%" >
			<tr>
				...
				<div class="search-condition clearfix">
			      	...
			      	<div id="con2"><div class="field">编号</div><input type="text" class="txt-input" id="QT_codeNum" name="QT_num" style="width:180px" /></div>
			      	<div id="con3"><div class="field">类别编号</div><input type="text" class="txt-input" id="QT_typeCode" name="QT_type" style="width:180px" /></div>
			        ...
			        <div id="con6" style="display: none"><div class="field">上级代码编号</div><input type="text" class="txt-input" id="QT_parentCodeNum" name="QT_parentCodeNum" style="width:180px"/></div>
			    	<div id="condition"  onmouseout="javascript:out()" onmouseover="javascript:over()" class="tout-chose" style="display:none;">
			      		<table border="0" class="other-chose">
			      			...
			      			<tr>
			      				<td><input type="checkbox" checked id="chk2" onClick="javascript:checkCondition(this)"/><label for="chk2">代码编号</label></td>
		      				</tr>
		      				<tr>
			      				<td><input type="checkbox" checked id="chk3"  onClick="javascript:checkCondition(this)"/><label for="chk3">类别编号</label></td>
		      				</tr>
		      				<tr>
		      				...
		      				<tr>
			      				<td><input type="checkbox" id="chk6"  onClick="javascript:checkCondition(this)"/><label for="chk6">上级代码编号</label></td>
		      				</tr>
			      		</table>
			      </div>
			...
			<tr>
				<td valign="top"><iframe id="mainFrame" name="mainFrame"
					src="<%=request.getContextPath()%>/view/paramCodeTreeListTemplate/Do-list.action?QT_codeNum=<%=request.getAttribute("QT_codeNum") %>&QT_typeCode=<%=request.getAttribute("QT_typeCode") %>&QT_parentCodeNum=<%=request.getAttribute("QT_parentCodeNum") %>&nodeId=<%=request.getAttribute("nodeId") %>"
