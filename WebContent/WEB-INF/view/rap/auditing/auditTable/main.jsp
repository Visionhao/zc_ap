...
<title>审核工作台(快速)</title>
<%@ include file="./import.jsp"%>
<script type="text/javascript">
$(function() {
	initView();
	var box, boxes, _i, _len;
    boxes = document.querySelectorAll('.box');
    for (_i = 0, _len = boxes.length; _i < _len; _i++) {
        box = boxes[_i];
        box.addEventListener('click', function() {
        	if($("#checkBox").attr("class")=='box'){//全选
        		$("input[name='chk_list']").attr("checked",true);
        	}else{//反选
        		$("input[name='chk_list']").attr("checked",false);
        	}
            return this.classList.toggle('ticked');
        });
    }
}); 

//标签名称(01 端口标签,02内容标签,03行为标签)type_code=XF0033
...
function initView(){
	...
	var showStyle='style=cursor:pointer;';
 	$.post("${pageContext.request.contextPath}/view/rap/auditing/auditTable/AuditTableAction-membersAuditTask.action",function(auditTaskList){
...
<body>
  <div class="container">
		<div class="head" align="center"><div style="padding-top: 10px;">审核工作台(快速)<span style="float: right;text-align: center;font-size: 12px;margin-top:10px;margin-right: 10px;font-family: fantasy;">欢迎您&nbsp;&nbsp;${memberName}</span></div></div>
