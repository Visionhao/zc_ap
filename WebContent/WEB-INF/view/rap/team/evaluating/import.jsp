...
<link href="<%=request.getContextPath() %>/view/rap/css/evaluating/evaluating.css" rel="stylesheet" type="text/css" />
<script src="${pageContext.request.contextPath}/view/rap/js/this.js" type="text/javascript"></script>
<script src="${pageContext.request.contextPath}/view/rap/js/evaluation/evaluation.js" type="text/javascript"></script>
...
//弹出提示框,需传入标题及内容
function mp_alter2(title,content,funname){
	$( "#dialog:ui-dialog" ).dialog( "destroy" );
	$("#dialog-message").remove();
	$("body").append('<div id="dialog-message" style="display:none;text-align:center" title="'+title+'"><p>'+content+'</p></div>');
	$( "#dialog-message" ).dialog({
...
function showdialogComponent(title,width,height,html){
	$( "#dialog:ui-dialog" ).dialog( "destroy" );
	 ...
	 $("body").append("<div id='dialogComponentMessage' style='display:none;'>"+html+"</div>");
